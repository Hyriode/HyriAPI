    ...
    public void sendData() {
        if (this.withHyggdrasil()) {
            ...
            if (type == HyggApplication.Type.PROXY) {
                ...
                packetProcessor.request(HyggChannel.SERVERS, new HyggServerInfoPacket(HyggServerState.valueOf(server.getState().name()), server.getPlayers(), server.getStartedTime(), new HyggServerOptions(), server.getData(), server.getSlots())).exec();
            }
        }
    }

package fr.hyriode.api;

/**
 * Project: HyriAPI
 * Created by AstFaster
 * on 10/03/2022 at 18:18
 */
public class HyriConstants {

    /** The static name field of the server */
    public static final String SERVER_NAME = "Hyriode";
    /** The static hostname field of the server */
    public static final String SERVER_IP = "hyriode.fr";
    /** The url of the server's website */
    public static final String WEBSITE_URL = "www.hyriode.fr";
    /** The url of the server's store */
    public static final String STORE_WEBSITE_URL = "hyriode.fr/store";
    /** The url of the server's Discord */
    public static final String DISCORD_URL = "discord.hyriode.fr";

}
